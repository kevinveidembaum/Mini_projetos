...
var pokemonHolder;
var hasAnErrorOnTheRequest = false; 

async function requestPokemon(url, name){
    await fetch(url + name)
        .then(response => response.json())
        .then(data => {pokemonHolder = data;})    
        .catch(err => {console.log(err); hasAnErrorOnTheRequest = true;});
...
if(!myTeam){
    ...
    setTimeout(() => {
        if(hasAnErrorOnTheRequest || pokemonHolder == null){
            displayErrorOnRequestCard("show");
            hasAnErrorOnTheRequest = !hasAnErrorOnTheRequest;
            if(!myTeam){
                disableButtons();
            }
        }else{
